...
cassandra-stress write n=1000000 cl=one -mode native cql3 user=cassandra password=cassandra -schema keyspace="keyspace1" -pop seq=5000001..6000000 -node `hostname` -graph file=/tmp/stresstestwr200thrds.html title=stresstestwrite200thrds revision=stresstest_`date +%Y%m%d_%H%M%S`_`hostname -s` -log file=/tmp/cassandra_stress_test_logfile_`date +%Y%m%d_%H%M%S`_`hostname -s`
echo
date
